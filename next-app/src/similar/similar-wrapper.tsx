...
export default async function SimilarWrapperServer({ params }: SimilarWrapperProps) {
  ...
  const graphLabels: ChartData[] = importData.slice(0, params.type === WhiskeyMatching.CHEMICAL ? masterWhiskey.chemicals.length : masterWhiskey.flavours.length).map((data) => ({ id: data.id, name: data.name, type: params.type, value: 0 }));
