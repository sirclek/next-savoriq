...
export function Chip({ children }: ChipProps) {
  return (
    <div className="flex select-none items-center gap-1 rounded bg-accent px-2 py-1 text-sm font-semibold text-accent-foreground">{children}</div>
  );
}
...
export function ChipClose(props: ChipCloseProps) {
  return <Button {...props} className="h-6 w-6" variant="transparent" circle icon={<CloseIcon size="1.2rem" />} />;
}
